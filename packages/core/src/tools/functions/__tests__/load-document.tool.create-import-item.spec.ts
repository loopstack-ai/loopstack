...
describe('LoadDocumentTool', () => {
    ...
    beforeEach(async () => {
        ...
        jest.spyOn(loadDocumentTool as any, 'applyModifiers').mockImplementation(
    ...
    describe('createImportItem', () => {
        it('should return a single document when many is false', () => {
            ...
            const currentEntities: DocumentEntity[] = [{ id: '1', content: 'current' } as unknown as DocumentEntity];
            const previousEntities: DocumentEntity[] = [{ id: '1', content: 'previous' } as unknown as DocumentEntity];

            (_.isEqual as jest.Mock).mockReturnValue(false);

            // Act
            const result = loadDocumentTool.createImportItem(options, currentEntities, previousEntities);
        ...
        it('should mark as not changed when entities are equal', () => {
            ...
            const currentEntities: DocumentEntity[] = [{ id: '1', content: 'same' } as unknown as DocumentEntity];
            const previousEntities: DocumentEntity[] = [{ id: '1', content: 'same' } as unknown as DocumentEntity];
