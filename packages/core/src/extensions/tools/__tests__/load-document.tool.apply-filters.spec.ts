...
describe('LoadDocumentTool', () => {
  ...
  describe('applyFilters', () => {
    ...
    it('should filter items when filter is both provided', () => {
      ...
      jest
        .spyOn(functionCallService, 'runEval')
        .mockImplementation((_, context) => {
      ...
      expect(functionCallService.runEval).toHaveBeenCalledWith(options.filter, {
        item: mockDocuments[0],
      });
      expect(functionCallService.runEval).toHaveBeenCalledWith(options.filter, {
        item: mockDocuments[1],
      });
      expect(functionCallService.runEval).toHaveBeenCalledWith(options.filter, {
        item: mockDocuments[2],
      });
    });
