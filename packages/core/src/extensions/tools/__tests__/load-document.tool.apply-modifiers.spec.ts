...
describe('LoadDocumentTool', () => {
  ...
  describe('applyModifiers', () => {
    const mockDocuments: DocumentEntity[] = [
      {
        id: '1',
        contents: { title: 'Doc 1', text: 'Content 1' },
      } as DocumentEntity,
      {
        id: '2',
        contents: { title: 'Doc 2', text: 'Content 2' },
      } as DocumentEntity,
      {
        id: '3',
        contents: { title: 'Doc 3', text: 'Content 3' },
      } as DocumentEntity,
    ];
    ...
    it('should use default map function when no map is provided', () => {
      ...
      mockFunctionCallService.runEval.mockImplementation(
        (_, { entity }) => entity.contents,
      );
      ...
      expect(functionCallService.runEval).toHaveBeenCalledWith(
        '{ entity.contents }',
    ...
    it('should sort results when sort option is set to true', () => {
      ...
      mockFunctionCallService.runEval.mockImplementation(
        (_, { entity }) => entity.contents.title,
      );
    ...
    it('should apply map, flatten and sort together', () => {
      const mockDocuments2 = [
        {
          id: '1',
          contents: { items: [{ index: 3 }, { index: 2 }] },
        } as DocumentEntity,
        {
          id: '2',
          contents: { items: [{ index: 1 }, { index: 5 }] },
        } as DocumentEntity,
