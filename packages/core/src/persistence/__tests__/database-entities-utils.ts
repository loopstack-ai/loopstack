...
export async function setupTestEnvironment(
  ...
  const workspaceRepo = moduleRef.get<Repository<WorkspaceEntity>>(
    getRepositoryToken(WorkspaceEntity),
  );
  const namespaceRepo = moduleRef.get<Repository<NamespaceEntity>>(
    getRepositoryToken(NamespaceEntity),
  );
  const documentRepo = moduleRef.get<Repository<DocumentEntity>>(
