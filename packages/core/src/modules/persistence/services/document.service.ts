...
export const WhereCondition = z.union([
  z.record(z.any(), PropertyCondition),
  LogicalOperator,
  BracketOperator,
]);
...
export class DocumentService {
  ...
  public applyWhereCondition(
      ...
      queryBuilder.andWhere(new Brackets(qb => {
        ...
        condition.or.forEach(subCondition => {
          if (first) {
            ...
            paramIndex = this.applyWhereCondition(subQb, subCondition, paramIndex);
            qb.where(`(${subQb.getQuery().replace(/^SELECT .* WHERE /, '')})`, subQb.getParameters());
          } else {
            paramIndex = this.applySubCondition(qb, subCondition, paramIndex, 'or');
          }
        });
      }));
      return paramIndex;
    ...
    if ('brackets' in condition) {
      queryBuilder.andWhere(new Brackets(qb => {
        paramIndex = this.applyWhereCondition(qb, condition.brackets, paramIndex);
      }));
      return paramIndex;
    }
    ...
    for (const [field, value] of Object.entries(condition)) {
      if (typeof value === 'object' && value !== null) {
        paramIndex = this.applyFieldOperator(queryBuilder, field, value, paramIndex);
      } else {
  ...
  private applyFieldOperator(
    ...
    if ('not' in operator) {
      if (typeof operator.not === 'object' && operator.not !== null) {
        ...
        queryBuilder.andWhere(new Brackets(qb => {
          ...
          const subQb = this.documentRepository.createQueryBuilder();
          paramIndex = this.applyWhereCondition(subQb, { [field]: operator.not }, paramIndex);
          const whereClause = subQb.getQuery().replace(/^SELECT .* WHERE /, '');
        ...
        // Simple NOT equal
        const paramName = `param${paramIndex++}`;
        queryBuilder.andWhere(`${field} != :${paramName}`, { [paramName]: operator.not });
      }
      return paramIndex;
    }
    ...
    for (const [op, symbol] of Object.entries(comparisonOperators)) {
      if (op in operator) {
        const paramName = `param${paramIndex++}`;
        queryBuilder.andWhere(`${field} ${symbol} :${paramName}`, { [paramName]: operator[op] });
        return paramIndex;
      }
    }

    // Handle BETWEEN operator
    if ('between' in operator && Array.isArray(operator.between) && operator.between.length === 2) {
    ...
    if ('in' in operator && Array.isArray(operator.in)) {
      const paramName = `param${paramIndex++}`;
      queryBuilder.andWhere(`${field} IN (:...${paramName})`, { [paramName]: operator.in });
      return paramIndex;
    }

    if ('notIn' in operator && Array.isArray(operator.notIn)) {
      const paramName = `param${paramIndex++}`;
      queryBuilder.andWhere(`${field} NOT IN (:...${paramName})`, { [paramName]: operator.notIn });
      return paramIndex;
    }

    if ('any' in operator && Array.isArray(operator.any)) {
      const paramName = `param${paramIndex++}`;
      queryBuilder.andWhere(`${field} = ANY(:${paramName})`, { [paramName]: operator.any });
      return paramIndex;
    }
    ...
    if ('all' in operator && Array.isArray(operator.all)) {
      ...
      queryBuilder.andWhere(`${field} = ALL(:${paramName})`, { [paramName]: operator.all });
      return paramIndex;
    }
