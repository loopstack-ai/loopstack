...
describe('ObjectExpressionHandler', () => {
  ...
  describe('security validation', () => {
    const testCases = [
      ...
      {
        expression: '${ user.__proto__ }',
        description: 'nested __proto__ access',
      },
  ...
  describe('valid segment patterns', () => {
    ...
    it('should accept valid segment patterns', () => {
      expect(() =>
        handler.process('${ user_name }', mockVariables),
      ).not.toThrow();
      expect(() =>
        handler.process('${ user-email }', mockVariables),
      ).not.toThrow();
      expect(() =>
        handler.process('${ items[0] }', mockVariables),
      ).not.toThrow();
      expect(() =>
        handler.process('${ data123.value }', mockVariables),
      ).not.toThrow();
    });
  ...
  describe('edge cases', () => {
    ...
    it('should handle numeric property names', () => {
      ...
      expect(handler.process('${ nested.456 }', variables)).toBe(
        'nested numeric key',
      );
    });
    ...
    it('should handle complex nested structures', () => {
      ...
      expect(handler.process('${ data.users[0].name }', variables)).toBe(
        'Alice',
      );
      expect(handler.process('${ data.users[1].roles[0] }', variables)).toBe(
        'user',
      );
    });
  });
