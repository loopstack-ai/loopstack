...
export class ObjectExpressionHandler implements TemplateDetector, TemplateProcessor {
  ...
  process(content: string, variables: Record<string, any>): any {
    try {
      ...
      this.logger.error(`Failed to process object expression (${content}): ${error.message}`, {
        expression: content,
      });
  ...
  private validateExpression(expression: string, options: ValidationOptions = {}): void {
    const {
