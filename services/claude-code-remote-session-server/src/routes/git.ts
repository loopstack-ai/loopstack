...
// ---------------------------------------------------------------------------
// POST /clone  { url, branch?, token? }
// ---------------------------------------------------------------------------

router.post('/clone', async (req: Request, res) => {
  try {
    const { url, branch, token } = req.body as { url?: string; branch?: string; token?: string };
    if (!url) {
    ...
    // Clone into the workspace root (must be empty). A token, if given, is supplied via GIT_ASKPASS —
    // never embedded in the URL, so it can't leak into process listings or logs.
    const branchArg = branch ? ` --branch ${branch}` : '';
    const result = await gitExec(`clone${branchArg} ${quotePath(url)} .`, 120_000, token);
