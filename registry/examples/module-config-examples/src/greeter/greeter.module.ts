...
export class GreeterModule {
  ...
  static forFeature(config: GreeterConfig): DynamicModule {
    return {
      ...
      providers: [{ provide: GREETER_CONFIG, useValue: config }, ...TOOLS],
      // GREETER_CONFIG is exported as well as the tools: a wrapper module that imports this one
      // and declares its own tools (see GreeterAgentModule) needs the scoped config to resolve,
      // otherwise those tools silently fall back to the global forRoot defaults.
      exports: [GREETER_CONFIG, ...TOOLS],
    };
  }
}
