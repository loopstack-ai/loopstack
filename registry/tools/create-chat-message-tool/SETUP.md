...
```bash
loopstack install @loopstack/create-chat-message-tool
```
...
1. Add `CreateChatMessageModule` to the imports of your module (e.g. `default.module.ts`):

```typescript
import { CreateChatMessageModule } from '@loopstack/create-chat-message-tool';

@Module({
  imports: [CreateChatMessageModule],
})
