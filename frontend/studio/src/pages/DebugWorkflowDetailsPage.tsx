...
export default function DebugWorkflowDetailsPage() {
  ...
  const breadcrumbsData = [
    ...
    {
      label: workflowConfig?.title || workflowConfig?.workflowName || workflowBlockName || 'Workflow Details',
      current: true,
    },
  ];
    ...
    <MainLayout breadcrumbsData={breadcrumbsData}>
      {isLoading ? (
        ...
        <div className="flex h-[calc(100vh-8rem)] flex-col gap-4">
          <h1 className="text-3xl font-bold tracking-tight">{workflowConfig.title || workflowConfig.workflowName}</h1>
