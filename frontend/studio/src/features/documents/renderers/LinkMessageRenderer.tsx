...
import { useWorkflowStatus } from '@/hooks/useWorkflows.ts';
...
const LinkMessageRenderer: React.FC<LinkMessageRendererProps> = ({ document }) => {
  ...
  const status = mapWorkflowStateToStatus(workflowStatus.data?.status);
  // While the live status is still loading, leave `defaultExpanded` undefined so LinkCard defers
  // locking in its initial value — prevents collapsing waiting/running children.
  const initialExpanded = workflowStatus.isSuccess ? expanded === true && status === 'pending' : undefined;

  return <LinkCard href={href} label={label} status={status} embed={embed} defaultExpanded={initialExpanded} />;
};
