...
import { AuthRequiredRenderer, OAuthPromptRenderer } from '@/features/oauth';
...
const DocumentRenderer: React.FC<DocumentRendererProps> = ({ pipeline, workflow, document, isActive, isLastItem }) => {
  ...
  const render = () => {
    switch (widget) {
      ...
      case 'auth-required':
        return <AuthRequiredRenderer pipeline={pipeline} workflow={workflow} document={doc} isActive={isActive} />;
      default:
