...
export function CodeExplorerTree({ nodes, searchQuery = '', onSelectFile }: CodeExplorerTreeProps) {
  const filteredNodes = useMemo(() => filterTree(nodes, searchQuery), [nodes, searchQuery]);

  return (
    <ScrollArea className="h-full w-full">
      <div className="min-w-0 py-1" role="tree" aria-label="File tree">
        {filteredNodes.length === 0 ? (
          <p className="px-2 py-4 text-sm text-muted-foreground">No files match</p>
        ) : (
          filteredNodes.map((node) => (
            <CodeExplorerTreeNode key={node.id} node={node} depth={0} onSelectFile={onSelectFile} />
          ))
        )}
      </div>
    </ScrollArea>
